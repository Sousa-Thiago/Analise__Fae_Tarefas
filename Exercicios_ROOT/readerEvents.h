...
// Thu Oct 17 13:45:26 2024 by ROOT version 6.33.01
// from TTree Events/Events
// found on file: 051E9D22-4F30-8E49-8477-644E65768282.root
//////////////////////////////////////////////////////////

#ifndef readerEvents_h
#define readerEvents_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   UInt_t          nIsoTrack;
   Float_t         IsoTrack_dxy[12];   //[nIsoTrack]
   Float_t         IsoTrack_dz[12];   //[nIsoTrack]
   Float_t         IsoTrack_eta[12];   //[nIsoTrack]
   Float_t         IsoTrack_pfRelIso03_all[12];   //[nIsoTrack]
   Float_t         IsoTrack_pfRelIso03_chg[12];   //[nIsoTrack]
   Float_t         IsoTrack_phi[12];   //[nIsoTrack]
   Float_t         IsoTrack_pt[12];   //[nIsoTrack]
   Float_t         IsoTrack_miniPFRelIso_all[12];   //[nIsoTrack]
   Float_t         IsoTrack_miniPFRelIso_chg[12];   //[nIsoTrack]
   Int_t           IsoTrack_charge[12];   //[nIsoTrack]
   Int_t           IsoTrack_fromPV[12];   //[nIsoTrack]
   Int_t           IsoTrack_pdgId[12];   //[nIsoTrack]
   Bool_t          IsoTrack_isHighPurityTrack[12];   //[nIsoTrack]
   Bool_t          IsoTrack_isPFcand[12];   //[nIsoTrack]
   Bool_t          IsoTrack_isFromLostTrack[12];   //[nIsoTrack]
   UInt_t          nJet;
   ...
   UInt_t          nSubJet;
   Float_t         SubJet_btagCSVV2[7];   //[nSubJet]
   Float_t         SubJet_btagDeepB[7];   //[nSubJet]
   Float_t         SubJet_eta[7];   //[nSubJet]
   Float_t         SubJet_mass[7];   //[nSubJet]
   Float_t         SubJet_n2b1[7];   //[nSubJet]
   Float_t         SubJet_n3b1[7];   //[nSubJet]
   Float_t         SubJet_phi[7];   //[nSubJet]
   Float_t         SubJet_pt[7];   //[nSubJet]
   Float_t         SubJet_rawFactor[7];   //[nSubJet]
   Float_t         SubJet_tau1[7];   //[nSubJet]
   Float_t         SubJet_tau2[7];   //[nSubJet]
   Float_t         SubJet_tau3[7];   //[nSubJet]
   Float_t         SubJet_tau4[7];   //[nSubJet]
   UInt_t          nTau;
   ...
   Float_t         SV_dxy[8];   //[nSV]
   Float_t         SV_dxySig[8];   //[nSV]
   Float_t         SV_pAngle[8];   //[nSV]
   Int_t           SV_charge[8];   //[nSV]
   ...
   Int_t           SubJet_hadronFlavour[7];   //[nSubJet]
   UChar_t         SubJet_nBHadrons[7];   //[nSubJet]
   UChar_t         SubJet_nCHadrons[7];   //[nSubJet]
...
#ifdef readerEvents_cxx
readerEvents::readerEvents(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("051E9D22-4F30-8E49-8477-644E65768282.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("051E9D22-4F30-8E49-8477-644E65768282.root");
      }
...
Int_t readerEvents::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t readerEvents::LoadTree(Long64_t entry)
{
...
void readerEvents::Init(TTree *tree)
{
...
void readerEvents::Show(Long64_t entry)
{
...
Int_t readerEvents::Cut(Long64_t entry)
{
