...
void Z_mass_after_cuts() {
  std::vector<std::string> diretorios = {
    ...
    // Histogramas com faixa focada em torno da massa do Z (80 a 100 GeV)
     ...
     for (const auto& diretorio : diretorios) {
        for (const auto& entry : std::filesystem::directory_iterator(diretorio)) {
            ...
            while (reader.Next()) {
                ...
                // Ordenar pT de forma decrescente
                std::sort(leptons.rbegin(), leptons.rend());

                // Seleciona os dois leptons com maior pT
                if (leptons.size() >= 2) {
                    ...
                    // Atribuir valores de pt, eta, phi de cada lepton
